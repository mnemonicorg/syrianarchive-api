...
import {filter, partial, keys, map, flow, intersection, identity, compact, pickBy, size, merge, orderBy, reduce, concat, curry, isEqual, keyBy, xor, take, find} from 'lodash/fp';
...
const units = map(i => merge(i, {
  ...
  units: map(u => u.substring(0, 8))(compact(find(u => u.incident_code === i.incident_code, rels).units.split(',')))
}))(is);
