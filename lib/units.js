...
import {filter, partial, keys, map, flow, intersection, identity, pickBy, size, merge} from 'lodash/fp';
...
export const list = (b) => {
  ...
  if (term) {
    ...
    uu = map(u => merge(u.original, {searchresult: u.string}), fuzzy.filter(term, units, options));
  } else {
    uu = units;
  }
