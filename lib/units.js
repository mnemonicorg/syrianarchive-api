...
import {filter, partial, keys, map, flow, intersection, identity, pickBy, size, merge, sortBy, reverse, reduce, concat, curry, isEqual, keyBy, xor, take} from 'lodash/fp';
...
const topdogs = (us) => take(100, reverse(sortBy('score', us)));
...
export const list = (b) => {
  ...
  if (term) {
    ...
    const rs = reduce(
      (a, t) => {
        const ff = topdogs(fuzzy.filter(t, units, options));
        return concatManyWith('original.reference_code', isEqual, mg, a, ff);
      }
    , [])(ts);

    uu = map(u => merge(u.original, {searchresult: u.string}), topdogs(rs));
  } else {
    uu = units;
  }
